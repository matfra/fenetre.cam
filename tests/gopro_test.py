...
class TestGoProPresetValidation(unittest.TestCase):
    ...
    def test_validate_presets_success(self, mock_logging, mock_get):
        ...
        gopro_cam = gopro.GoPro(
            preset_day={'id': 65536},
            preset_night={'id': 65539}
        )
        gopro_cam.validate_presets()

        mock_logging.info.assert_called_with("Available presets: ['Standard Photo', 'Night Photo']")
    ...
    def test_validate_presets_failure_invalid_id(self, mock_logging, mock_get):
        ...
        gopro_cam = gopro.GoPro(
            preset_day={'id': 12345},  # Invalid ID
            preset_night={'id': 65539}
        )
        gopro_cam.validate_presets()

        mock_logging.error.assert_any_call("Configured day preset ID '12345' is not available on the camera.")

    @mock.patch('gopro.requests.get')
    @mock.patch('gopro.logging')
    def test_validate_presets_failure_no_presets_retrieved(self, mock_logging, mock_get):
        ...
        mock_logging.error.assert_called_with("Could not retrieve available presets from the camera.")
    ...
    def test_capture_photo_with_preset_settings(self, mock_get, mock_get_latest_file):
        ...
        expected_calls = [
            ...
            mock.call('http://10.5.5.9/gopro/camera/presets/load?id=65536', timeout=5, verify=''),
            mock.call('http://10.5.5.9/gopro/camera/setting?option=3&setting=125', timeout=5, verify=''),
