...
from PIL import Image, ImageDraw # Ensure Image is imported if spec=Image.Image is used
import numpy as np
...
class TestDaylightProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_camera_data_path = "test_data/test_camera" # Added from previous context
    ...
    def test_get_avg_color_success(self):
        ...
        mock_img_input = mock.MagicMock(spec=Image.Image) # Input image to get_avg_color
        mock_sky_region_cropped = mock.MagicMock(spec=Image.Image) # Result of img.crop()
        ...
        with mock.patch('daylight.np.array', return_value=mock_array_object_instance) as mock_np_array_function:
            # Call the function under test
            avg_color_result = daylight.get_avg_color(mock_img_input, (0, 0, 10, 10))
            
            # Assertions
            self.assertEqual(avg_color_result, (100, 150, 200)) # Expecting integers after conversion
    ...
    def test_get_avg_color_error_on_crop(self):
        ...
        mock_img_input.crop.side_effect = Exception("Test crop failure") # Simulate error during crop
    ...
    def test_get_avg_color_error_on_numpy_operation(self):
        ...
        with mock.patch('daylight.np.array', side_effect=Exception("Simulated Numpy error")):
            avg_color_result = daylight.get_avg_color(mock_img_input, (0, 0, 10, 10))
            self.assertEqual(avg_color_result, self.default_sky_color)
        ...
        mock_mean_method_error = mock.MagicMock(side_effect=Exception("Simulated mean error"))
        ...
        with mock.patch('daylight.np.array', return_value=mock_array_object_instance_for_mean_error):
            avg_color_result = daylight.get_avg_color(mock_img_input, (0, 0, 10, 10))
            self.assertEqual(avg_color_result, self.default_sky_color)
    ...
    def test_create_daily_band_no_images(self, mock_pil_draw, mock_pil_new, mock_os_listdir):
        ...
        mock_pil_new.assert_called_once_with("RGB", (1, self.daily_band_height), self.default_sky_color)
    ...
    def test_create_monthly_image_mixed_days(self, mock_makedirs, mock_pil_new, mock_pil_open, mock_os_path_exists, mock_monthrange):
        ...
        def os_path_exists_side_effect(path):
            if "2023-02-01/daylight.png" in path: return True
            if "2023-02-02/daylight.png" in path: return False
            if "2023-02-03/daylight.png" in path: return True
            if "2023-02-04/daylight.png" in path: return True
            if "2023-02-05/daylight.png" in path: return True
            return False 
        mock_os_path_exists.side_effect = os_path_exists_side_effect

        def pil_open_side_effect(path):
            if "2023-02-01/daylight.png" in path: return mock_valid_band
            if "2023-02-03/daylight.png" in path: return mock_invalid_dim_band
            if "2023-02-04/daylight.png" in path: raise OSError("Cannot open image")
            if "2023-02-05/daylight.png" in path: return mock_valid_band
        ...
        def pil_new_side_effect(mode, size, color=None):
            ...
            elif size == (1, self.daily_band_height) and color == self.default_sky_color: 
        ...
        expected_save_path = os.path.join(self.test_camera_data_path, "daylight", f"{year_month_str}.png")
        
        result_path = daylight.create_monthly_image(year_month_str, self.test_camera_data_path)

        self.assertEqual(result_path, expected_save_path)
        mock_monthrange.assert_called_once_with(2023, 2)
        mock_makedirs.assert_called_once_with(os.path.join(self.test_camera_data_path, "daylight"), exist_ok=True)
        
        # Count how many times a default band was supposed to be created by Image.new
        num_default_bands_created_by_new = sum(
            1 for ci in created_images_log if ci['size'] == (1, self.daily_band_height) and ci['color'] == self.default_sky_color
        )
    ...
    def test_list_valid_days_directories(self, mock_os_listdir, mock_os_path_isdir):
        ...
        mock_os_listdir.return_value = ["2023-10-01", "2023-10-02", "invalid-dir", "2023-09-30", "not_a_dir.txt"]
        
        def isdir_side_effect(path):
            if path.endswith("not_a_dir.txt"): return False
            return True 
